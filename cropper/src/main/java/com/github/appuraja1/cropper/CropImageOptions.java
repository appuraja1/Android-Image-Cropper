 ...
 * Initialized with safe defaults and robust Parcelable implementation.
 */
public class CropImageOptions implements Parcelable {
  ...
  protected CropImageOptions(Parcel in) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
      outputUri = in.readParcelable(Uri.class.getClassLoader(), Uri.class);
    } else {
      outputUri = in.readParcelable(Uri.class.getClassLoader());
    }

    String compressFormatStr = in.readString();
    outputCompressFormat = compressFormatStr != null
            ? Bitmap.CompressFormat.valueOf(compressFormatStr)
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
      initialCropWindowRectangle = in.readParcelable(Rect.class.getClassLoader(), Rect.class);
    } else {
  ...
  public void writeToParcel(@NonNull Parcel dest, int flags) {
    ...
    dest.writeByte((byte) (noOutputImage ? 1 : 0)); // Fixed: was dest.writeInt
