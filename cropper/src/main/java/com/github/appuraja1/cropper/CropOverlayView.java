...
/**
 * Modern Custom View representing the crop window overlay, grid guidelines,
 * corner handles, and shadow background outside the crop selection.
 */
public class CropOverlayView extends View {
  ...
  public void setAspectRatioX(int aspectRatioX) {
    if (aspectRatioX <= 0) {
      throw new IllegalArgumentException("Cannot set aspect ratio value to a number <= 0.");
  ...
  /**
   * Modern, crash-free background drawing using clean geometry / clipOut APIs.
   */
  private void drawBackground(Canvas canvas) {
    ...
    if (mCropShape == CropImageView.CropShape.RECTANGLE) {
      if (!isNonStraightAngleRotated()) {
        // Fast 4-quadrant draw without expensive clipping operations
  ...
  private void drawGuidelines(Canvas canvas) {
    if (mGuidelinePaint != null) {
      ...
      if (mCropShape == CropImageView.CropShape.OVAL) {
        ...
        float yv = (float) (h * Math.sin(Math.acos(Math.min(1.0, Math.max(-1.0, (w - oneThirdCropWidth) / w)))));
        ...
        float xv = (float) (w * Math.cos(Math.asin(Math.min(1.0, Math.max(-1.0, (h - oneThirdCropHeight) / h)))));
